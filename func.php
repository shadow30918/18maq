...
output : OK(1=成功,0=失敦), cityRows(縣市資訊), storeRows(櫃點資訊), total(目前該活動參加人數)
=========================================================================
func=SubmitForm
接收登記者資料
...
-------------------------------------------------------------------------
output : OK(1=成功),
output : info(array, [OK=0 && (status=3 || 4)] || OK=1) 包含 ==> city(縣市名稱), store(櫃點名稱), name(登記者姓名), mobile(登記者手機), email(登記者Email), expire_date(兌換失效時間)
=========================================================================
func=StoreSubmit
櫃點兌換確認兌換
input : sn (透過網址取得)
...
output : OK(1=成功)
=========================================================================
func=getTotalTester (Add@2018-04-04)
確認該使用者有參加這個活動, 並回傳參加該活動人數
input : c_no(縣市編號), s_no(櫃點編號), t_no(登記者編號), token (由 SubmitForm 成功時所得到的 token)
output : OK(0=失敗), status(1=有欄位沒填 / 2=有欄位格式不正確 / 3=token錯誤 / 4=縣市跟櫃點對不起來 / 5=查無該名登記者), total(目前該活動參加人數)
*/
...
//兌換頁活動網址
$exchange_url="https://www.shiseido-event.com/2018maquillage/exchange.html";
		...
		if ($json==null) {
			switch ($func) {
				case "CityStoreListByEvent":
					...
					$sqlStr='SELECT COUNT(*) AS TotalTeasterForSingleEvent FROM Tester WHERE t_e_no=:event_no';
					$db->query($sqlStr);
					$db->bind(':event_no', $event_no, PDO::PARAM_INT);
					$rows = $db->resultset();
					$TotalTeasterForSingleEvent=$rows[0]['TotalTeasterForSingleEvent'];

					$json=array('OK'=>1,'cityRows'=>$cityRows,'storeRows'=>$storeRows, 'total'=>$TotalTeasterForSingleEvent);
					break;
				case "SubmitForm":
				...
				case "getTotalTester":
					if (isset($_POST['c_no']) && isset($_POST['s_no']) && isset($_POST['t_no']) && isset($_POST['token'])) {
						...
						if ($return_OK==0) {
							...
							$json=array('OK'=>'1', 'status'=>'0', 'total'=>$TotalTeasterForSingleEvent);
						}
					}
					break;
			}
